import { Menu, Search, Mic, Video, Bell, X } from "lucide-react";

export default function Navbar({ onMenuClick, searchOpen, setSearchOpen }) {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex h-14 items-center justify-between gap-4 bg-white px-2 sm:px-4">
      {/* Left: menu + logo */}
      <div className={`flex items-center gap-1 sm:gap-2 ${searchOpen ? "hidden sm:flex" : "flex"}`}>
        ...
        <a href="#" className="flex items-center gap-1 pl-1">
          ...
          <span className="hidden text-xl tracking-tight text-gray-900 sm:inline">YouTube</span>
          <span className="hidden self-start text-[10px] text-gray-500 sm:inline">NG</span>
        </a>
      </div>
      ...
      >
        {searchOpen && (
          <button
            ...
            className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full hover:bg-gray-100 sm:hidden"
        ...
        <div className="flex w-full flex-1 items-center">
          <div className="flex h-10 flex-1 items-center rounded-l-full border border-gray-300 pl-4 focus-within:border-blue-500 focus-within:shadow-inner">
            <input
              type="text"
              placeholder="Search"
          ...
          <button
            aria-label="Search"
            className="flex h-10 w-16 flex-shrink-0 items-center justify-center rounded-r-full border border-l-0 border-gray-300 bg-gray-50 hover:bg-gray-100"
          >
        ...
        <button
          ...
          className="hidden h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 sm:flex"
        >
          <Mic size={20} />
        </button>
      </div>
      ...
      <div className={`flex flex-shrink-0 items-center gap-1 sm:gap-2 ${searchOpen ? "hidden sm:flex" : "flex"}`}>
        ...
        <button
          aria-label="Create"
          className="hidden h-10 w-10 items-center justify-center rounded-full hover:bg-gray-100 sm:flex"
        >
          <Video size={22} />
        </button>
        <button
